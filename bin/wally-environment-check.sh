...
if [[ "$ID" == rhel || "$ID_LIKE" == *rhel* ]]; then
    ...
    if (( SUSE_VERSION > 160 )); then
        printf "${WARNING_COLOR}%s\n${ENDC}" "The Wally installation script has only been tested with SUSE version 15.6 and 16.0. You have a newer version ($VERSION). The installation for SUSE 16.0 will be attempted, but there may be issues."
    fi
else
