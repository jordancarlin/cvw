...
case "$FAMILY" in
    ...
    suse) 
        if (( SUSE_VERSION >= 160)); then
            ...
            BOOST_VERSION=86
            VERILATOR_PACKAGES+=(mold) # Mold available starting in SUSE 16.0
        elif (( SUSE_VERSION >= 156 )); then
            PYTHON_VERSION=python3.12
            PYTHON_VERSION_PACKAGE=python312
            BOOST_VERSION=75
            BUILDROOT_PACKAGES+=(ncurses5-devel) # ncurses5 no longer available in SUSE 16.0
        ...
        SPIKE_PACKAGES+=(dtc "libboost_regex1_${BOOST_VERSION}_0-devel" "libboost_system1_${BOOST_VERSION}_0-devel")
